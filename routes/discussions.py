...
@router.get('/', response_model=List[ThreadRes])
def get_general_threads():
    return [ThreadRes.from_obj(obj) for obj in discussion.get_general_posts()]


@router.post('/', status_code=201, response_model=ThreadRes)
...
@router.post('/{guid}/comments', status_code=201, response_model=DiscussionRes)
